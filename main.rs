...
async fn connect(adapter: &bluer::Adapter, addr: bluer::Address) -> bluer::Result<()> {
    let device = adapter.device(addr)?;
    let mut retries = 5;
    loop {
        if retries <= 0 {
        ...
        if !device.is_trusted().await? {
            println!("[{addr}] Trusting...");
            if let Err(err) = device.set_trusted(true).await {
                println!("[{addr}] Error: {err}");
                retries -= 1;
                continue;
            } else {
                println!("[{addr}] Trusted.")
            }
        }
        if !device.is_connected().await? {
            println!("[{addr}] Connecting...");
            match device.connect().await {
                Ok(()) => {}
                Err(err) => {
                    println!("[{addr}] Connect error: {err}");
                    retries -= 1;
                    continue;
                }
            }
            ...
            println!("[{}] Already connected", addr);
            break;
        }
    }
    Ok(())
...
async fn main() -> bluer::Result<()> {
    // let hid_uuid = "00001124-0000-1000-8000-00805f9b34fb";
                    ...
                    AdapterEvent::DeviceAdded(addr) => {
                        ...
                        if let Err(err) = res {
                            ...
                            tokio::task::spawn(async move {
                                if let Err(err) = connect(&adapter_, addr_).await {
                                    println!("[{addr}] Connect error: {err}")
                                }
                            });
                        }
